...
/**
 * Redis-based caching service for microservices
 */
export class CacheService {
  ...
  async ttl(key: string): Promise<number> {
    if (!this.isReady) {
      logger.warn('Redis cache not ready, skipping ttl operation');
      return -1;
    }
  ...
  async increment(key: string, by: number = 1): Promise<number> {
    if (!this.isReady) {
      logger.warn('Redis cache not ready, skipping increment operation');
      return 0;
    }
